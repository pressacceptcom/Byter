...
# equivalent representation in hexadecimal.
#
# |------------------|
# | Meta Information |
# |------------------|
...
func _on_Decimal_text_changed() -> void:
	...
	if basic.negative_bool:
		text = Hexadecimal.signed2str(basic.to_decimal())
	else:
		text = Hexadecimal.unsigned2str(basic.to_decimal())
